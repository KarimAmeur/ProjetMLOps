...
page = st.sidebar.radio(
    "Sélectionnez une page", ["Régression Logistique", "Random Forest", "Prédiction"]
)
...
if page == "Régression Logistique":
    ...
    with col1:
        st.write(
            pd.DataFrame(lr_data["metrics"].items(), columns=["Métrique", "Valeur"])
        )
...
# Page 2: Random Forest
    ...
    with col1:
        st.write(
            pd.DataFrame(rf_data["metrics"].items(), columns=["Métrique", "Valeur"])
        )
...
# Page 3: Prédiction
    ...
    if model_type == "Régression Logistique":
        model_path = load_mlflow_experiments("logistic_regression")["artifacts"][
            "model_path"
        ]
    else:
    ...
    with col1:
        credit_lines = st.number_input(
            "Nombre de lignes de crédit", min_value=0, value=3
        )
        loan_amt = st.number_input(
            "Montant du prêt en cours ($)", min_value=0, value=15000
        )
    ...
    with col2:
        ...
        fico_score = st.number_input(
            "Score FICO", min_value=300, max_value=850, value=680
        )
